...
# Leer el archivo CSV
datos_sub <- read.csv("johnatan3/SUB_BASE3.csv", header = TRUE, sep = ",")
...
# Función para generar los puntos iniciales de todos los parámetros
# Función para generar los puntos iniciales de todos los parámetros basados en las medias posteriores
init_fn <- function() {
  list(
    # Inicialización de los parámetros alpha con sus medias posteriores
    alpha = c(201.70, 248.35, 286.35, 221.45, 205.74, 188.22, 224.40, 
              192.98, 184.60, 215.92, 204.50, 235.14, 305.50, 274.71),
    
    # Inicialización de los parámetros mu con sus medias posteriores
    mu = c(230.71, 221.31, 219.14, 235.42),
    ...
    horas_cen = c(4.20, -4.20),
    estrato_cen = c(6.23, 5.30, 2.51, -4.99, -5.62, -3.45),
    internet_cen = c(-2.87, 2.87),
    educ_madre_cen = c(-15.45, -9.97, -6.38, 5.53, 8.83, 17.45),
    educ_padre_cen = c(-12.42, -5.44, -3.64, 5.35, -0.36, 16.50),
    naturaleza_cen = c(-14.65, 14.65)
  )
}
...
fit <- stan(data = stan_data, 
            ...
            refresh = 3000,
            thin = 4,
            init = init_fn)  # Usar la función de inicialización definida
fit
options(max.print = 1050)
print(fit)
save(fit, file = "johnatan3/particion03_modelo_completo_sub3.RData")